...
/** Azure Neural TTS — REST API with subscription key (no SDK dep) */
async function azureTTS({ text, voice = 'zh-TW-HsiaoChenNeural', speed = 1.0, apiKey, region, emotion, style }) {
  if (!apiKey) throw new Error('Azure TTS API key required');
  const r = region || process.env.AZURE_SPEECH_REGION || 'eastasia'
  const url = `https://${r}.tts.speech.microsoft.com/cognitiveservices/v1`
  const rate = Math.min(Math.max(parseFloat(speed) || 1.0, 0.5), 2.0)
  const pct = `${Math.round((rate - 1) * 50)}%`

  // Build SSML with emotion + style support (繁中 zh-TW voices include HsiaoChen/HsiaoYu/YunYang)
  ...
  let prosody = ` rate="${pct}" pitch="0%"`
  let inner = safeText

  // Optional style/emotion tags — Azure supports limited styles per voice
  if (style && ['cheerful', 'sad', 'angry', 'fearful', 'disgruntled', 'serious', 'affectionate', 'gentle', 'embarrassed', 'calm', 'friendly', 'hopeful', 'shouting', 'terrified', 'unfriendly'].includes(style)) {
    inner = `<mstts:express-as style="${style}">${safeText}</mstts:express-as>`
  }
  ...
  <voice name='${voice}'><prosody${prosody}>${inner}</prosody></voice>
</speak>`
  ...
  if (!response.ok) {
    ...
    throw new Error(`Azure TTS error: ${response.status} ${errText.slice(0, 200) || response.statusText}`)
  }
...
/** Google Cloud TTS — REST API with API key (no SDK dep) */
async function googleTTS({ text, voice = 'cmn-TW-Wavenet-A', speed = 1.0, apiKey, emotion }) {
  if (!apiKey) throw new Error('Google Cloud TTS API key required');
  const url = `https://texttospeech.googleapis.com/v1/text:synthesize?key=${apiKey}`
  const rate = Math.min(Math.max(parseFloat(speed) || 1.0, 0.25), 4.0)

  // Map emotion → speaking rate / pitch adjustments (Google does not have emotion tags directly)
  let pitch = 0
  let speakingRate = rate
  if (emotion === 'excited') { pitch = 4; speakingRate = Math.min(rate * 1.1, 4) }
  else if (emotion === 'happy' || emotion === 'cheerful') { pitch = 2; speakingRate = rate }
  else if (emotion === 'sad') { pitch = -3; speakingRate = Math.max(rate * 0.9, 0.25) }
  else if (emotion === 'whisper') { pitch = -2; speakingRate = Math.max(rate * 0.85, 0.25) }
  else if (emotion === 'broadcast') { pitch = 1; speakingRate = rate * 1.05 }

  const body = {
    input: { text },
    voice: {
      languageCode: voice.startsWith('cmn') || voice.startsWith('zh') ? 'cmn-TW' : 'en-US',
      name: voice,
    },
    audioConfig: {
      audioEncoding: 'MP3',
  ...
  if (!response.ok) {
    const err = await response.json().catch(() => ({}))
    throw new Error(`Google TTS error: ${err?.error?.message || response.statusText} (${response.status})`)
  }
...
/** Dispatcher */
/**
 * @typedef {Object} SynthesizeParams
 ...
 */
/**
 * @param {SynthesizeParams} [params]
 */
async function synthesize(params = {}) {
  const { engine = 'openai', text, voice, speed, apiKey, emotion, style, region } = params
  ...
  switch (engine) {
    ...
    case 'azure':
      return azureTTS({ text, voice, speed, apiKey, emotion, style, region });
    case 'google':
      return googleTTS({ text, voice, speed, apiKey, emotion });
    default:
...
module.exports = {
  ...
  azureTTS,
  googleTTS,
  SUPPORTED_ENGINES: ['openai', 'elevenlabs', 'kokoro', 'azure', 'google'],
};
